...
CSP_DEFAULT_SRC = ("'self'",)  # By default, only allow resources from same origin
CSP_STYLE_SRC = ("'self'", "'unsafe-inline'", "https://cdn.jsdelivr.net")  # Allow styles from same origin, inline styles, and Bootstrap CDN
CSP_SCRIPT_SRC = ("'self'", "'unsafe-inline'", "https://cdn.jsdelivr.net", "https://js.stripe.com")  # Allow scripts from same origin, inline scripts, Bootstrap CDN, and Stripe
CSP_IMG_SRC = ("'self'", "data:", "https:")  # Allow images from same origin, data URIs, and any HTTPS source
CSP_CONNECT_SRC = ("'self'", "https://api.stripe.com")  # Allow API connections to same origin and Stripe API
